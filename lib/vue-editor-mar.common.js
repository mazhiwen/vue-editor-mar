...
var lokijs = __webpack_require__(3);
var lokijs_default = /*#__PURE__*/__webpack_require__.n(lokijs);

// CONCATENATED MODULE: ./src/packages/edit/config.js
var wordTypesList = [{
  type: 'keyword',
  words: ['function', 'SELECT', 'DISTINCT', 'FROM', 'AS', 'ON', 'INNER', 'JOIN', 'LEFT', 'WHERE', 'IS', 'NOT', 'NULL', 'ORDER', 'BY', 'PARTITION', 'over', 'DESC', 'type', 'this', 'const']
}, {
  type: 'condition',
  words: ['if', 'else', 'while']
}, {
...
var db = new lokijs_default.a('Example');
var keywordsCollection = null;

var setConfigToMentionData = function setConfigToMentionData(wordTypesListV) {
  var keywordsList = [];
  wordTypesListV.forEach(function (value, index) {
    value.words.forEach(function (valueW, indexW) {
      keywordsList.push({
        text: valueW
      });
    });
...
setConfigToMentionData(wordTypesList); // const dv = keywords.addDynamicView('a_complex_view');
...
var EditorDataController_DataController = function DataController(_ref) {
  ...
  this.editWrapRect = {};
  this.inputTexareaDom = null; // 编辑框ref

  this.editRef = null; // 数字测试ref

  this.fonttestNumRef = null; // 中文测试ref

  this.fonttestCNRef = null; // 行dom元素
  ...
  this.editLetPadding = 4; // 行高度

  this.lineHeight = 25; // 提示框数据
  ...
  this.hasInit = false;
  this.wordTypesList = copy_default.a.deepCopy(wordTypesList);
};

EditorDataController_DataController.prototype.mergeConfig = function (_ref2) {
  var wordType = _ref2.wordType;
  var wordTypesListT = copy_default.a.deepCopy(wordTypesList);

  if (wordType) {
    var wordTypeKeyList = Object.keys(wordType);
    wordTypesListT.forEach(function (value, index) {
      if (wordTypeKeyList.includes(value.type)) {
        value.words = /* Cannot get final name for export "util" in "./node_modules/utility-mar/index.js" (known exports: , known reexports: utiDate cookie copy dataFormat validator download dataStructure) */ undefined.unionArray(value.words, wordType[value.type]);
      }
    });
  }
...
EditorDataController_DataController.prototype.getBoundingData = function () {
...
EditorDataController_DataController.prototype.setOnDraggingData = function (data) {
...
EditorDataController_DataController.prototype.appendCurrentToList = function () {
  this.appendDataToList(this.currentDraggingData);
}; // 添加字符数据到行末尾


EditorDataController_DataController.prototype.appendDataToList = function (data) {
...
EditorDataController_DataController.prototype.insertCurrentDraggingToList = function (index) {
...
EditorDataController_DataController.prototype.spliceUnitToList = function (_ref3) {
  var _this$editUnitListDat,
      _this = this;

  var spliceParams = _ref3.spliceParams,
      isGoToLastUnit = _ref3.isGoToLastUnit,
      isAddTxt = _ref3.isAddTxt,
      data = _ref3.data,
      isAddNewUnit = _ref3.isAddNewUnit,
      newTxt = _ref3.newTxt;
  ...
  setTimeout(function () {
    ...
    currentUnitData.text.split('').forEach(function (value) {
      textWidth += _this.getCharacterWidth(value);
    });
...
EditorDataController_DataController.prototype.resetArrowKeyStartIndex = function () {
  this.arrowKeyStartFocusUnitIndexInRow = null;
}; // 插入字符数据到行数据 数值操作,并且更新光标位置


EditorDataController_DataController.prototype.editText = function (_ref4) {
  var _this2 = this;

  var isAddTxt = _ref4.isAddTxt,
      newTxt = _ref4.newTxt;
  this.resetArrowKeyStartIndex();
  setTimeout(function () {
    // 添加
    if (isAddTxt) {
      _this2.insertTxts(newTxt);
    } else {
      // 删除
      _this2.deleteTxt();
    }
...
EditorDataController_DataController.prototype.updateCharacterLengthData = function (spliceParams) {
...
EditorDataController_DataController.prototype.initTemporaryInsertIndex = function () {
  this.temporaryInsertIndex = null;
};

EditorDataController_DataController.prototype.clearCurrentDragData = function () {
...
EditorDataController_DataController.prototype.cancelDrag = function () {
...
EditorDataController_DataController.prototype.setStartMoveData = function () {
...
EditorDataController_DataController.prototype.setStartMouseDownPosition = function (e, focusRowIndex) {
...
EditorDataController_DataController.prototype.clearStartMouseDownPosition = function () {
  this.isMouseSelectMoving = false;
}; // 聚焦行末尾


EditorDataController_DataController.prototype.focusEndRow = function (index) {
...
EditorDataController_DataController.prototype.initSelectLine = function () {
  this.selectArea = [];
};

EditorDataController_DataController.prototype.focusHead = function () {
  this.cursorData = {
...
EditorDataController_DataController.prototype.initFocus = function () {
...
EditorDataController_DataController.prototype.focusTextarea = function () {
  this.inputTexareaDom && this.inputTexareaDom.focus();
}; // 根据屏幕点 定焦文字单元


EditorDataController_DataController.prototype.focusUnit = function (_ref5) {
  var _this3 = this;

  var focusUnitIndexInRow = _ref5.focusUnitIndexInRow,
      focusRowIndex = _ref5.focusRowIndex,
      unitRect = _ref5.unitRect,
      compareParameter = _ref5.compareParameter,
      isFocus = _ref5.isFocus;
  ...
  unitData.text.split('').every(function (value, index) {
    ...
    var currentCharacterWidth = _this3.getCharacterWidth(value);
    ...
    if (compareParameter <= left + currentCharacterWidth / 2) {
      ...
      _this3.cursorData.top = focusRowIndex * _this3.lineHeight;
...
EditorDataController_DataController.prototype.getUnitRect = function (focusRowIndex, focusUnitIndexInRow) {
  var rowChildNodes = this.rowDomRefs[focusRowIndex].childNodes;
  return rowChildNodes[focusUnitIndexInRow].getBoundingClientRect();
}; // 根据字符index 定焦光标文字单元


EditorDataController_DataController.prototype.focusUnitByIndex = function (focusRowIndex, focusUnitIndexInRow, focusChaIndexInUnit) {
  var _this4 = this;
  ...
  if (focusUnitIndexInRow === -1) {
    ...
    unitData.text.split('').every(function (value, index) {
      ...
      left += _this4.getCharacterWidth(value);
...
EditorDataController_DataController.prototype.getIsMouseMoving = function (e) {
...
EditorDataController_DataController.prototype.onUnitMouseDown = function (focusUnitIndexInRow, focusRowIndex, unitRect, e) {
...
EditorDataController_DataController.prototype.onUnitMouseOver = function (focusUnitIndexInRow, focusRowIndex, unitRect, e) {
  var _this5 = this;
      ...
      if (disRow > 1) {
        ...
        while (countRowIndex < highIndex) {
          selectArea.push({
            ...
            top: countRowIndex * _this5.lineHeight
          });
          countRowIndex += 1;
        }
      }
...
EditorDataController_DataController.prototype.onUnitMouseUp = function (focusUnitIndexInRow, focusRowIndex, unitRect, e) {
  console.log('onUnitMouseUp');
  this.clearStartMouseDownPosition();
  this.focusTextarea(); // this.initSelectLine();
};

EditorDataController_DataController.prototype.onRowMouseDown = function (e, focusRowIndex) {
...
EditorDataController_DataController.prototype.onRowMouseUp = function (e, rowIndex) {
...
EditorDataController_DataController.prototype.getUnitType = function (txt) {
  if (/^\d+$/.test(txt)) {
    return 'number';
  }
  ...
  if (/^\S+$/.test(txt)) {
    var type = null;
    this.wordTypesList.forEach(function (value, index) {
...
EditorDataController_DataController.prototype.deleteTxt = function () {
  var _this6 = this;

  var rowData = this.textData[this.focusRowIndex]; // console.log('deleteTxt前', this.focusUnitIndexInRow, this.focusChaIndexInUnit);
  // 光标到达行开头
  ...
  var replaceUnit = function replaceUnit() {
    ...
    data.text = newText;
    data.type = _this6.getUnitType(newText);
  ...
  if (this.focusChaIndexInUnit === 0) {
    if (this.focusUnitIndexInRow === 0) {
      ...
      if (arr.length === 1) {
        ...
        if (rowData.length >= this.focusUnitIndexInRow + 2) {
          ...
          var newType = this.getUnitType(newText);
    ...
    var _newText = replaceUnit();

    this.focusChaIndexInUnit -= 1;
    this.setMentionData(_newText);
...
var unitConnect = function unitConnect(unitAtype, unitBtype, combineFn, splitFn) {
  ...
  if (unitAtype === 'space') {
    ...
    splitFn();
  } else if (unitBtype === 'space' || unitBtype === 'split' || unitBtype === 'operator') {
    splitFn();
  } else {
    combineFn();
  }
...
var getIsRegularType = function getIsRegularType(type) {
  return type !== 'space' && type !== 'split' && type !== 'operator';
}; // 解析字符串生成 unit数组,并更新对应光标，以及私有属性


EditorDataController_DataController.prototype.parseStrToUnitArr = function (_ref6) {
  var _this7 = this;
    ...
    if (type) {
      if (lastUnitData) {
        if (type === 'breakline') {
          ...
          unitConnect(lastUnitData.type, type, function () {
            focusChaIndexInUnit += 1;
            newTxt = "".concat(lastUnitData.text).concat(value);
            lastUnitData.text = newTxt;
            lastUnitData.type = _this7.getUnitType(newTxt);
          }, function () {
            focusUnitIndexInRow += 1;
...
EditorDataController_DataController.prototype.addTxt = function (_ref7) {
  var _this8 = this;
  ...
  rowData = this.textData[this.focusRowIndex]; // this.textData[this.focusRowIndex];

  var txtType = this.getUnitType(newTxt);
  var data = {
    type: txtType
  }; // 在末尾 append 一个新的unit
  ...
  var mentionText = null; // if (this.focusUnitIndexInRow === null) { // 初始化无数据
  //   newAppend();
  // } else
  ...
  if (this.focusUnitIndexInRow === -1) {
    ...
    // 在行中间或者尾部插入数据
    var focusUnitData = rowData[this.focusUnitIndexInRow];
    var focusText = focusUnitData.text;
    var focusTextHead = focusText.slice(0, this.focusChaIndexInUnit + 1);
    var focusTextTail = focusText.slice(this.focusChaIndexInUnit + 1);
    var focusTxtType = focusUnitData.type;
    ...
    var splitToThreeUnit = function splitToThreeUnit() {
      rowData.splice(_this8.focusUnitIndexInRow, 1, {
        text: focusTextHead,
        type: _this8.getUnitType(focusTextHead)
      }, {
        text: newTxt,
        type: txtType
      }, {
        text: focusTextTail,
        type: _this8.getUnitType(focusTextTail)
      });
      _this8.focusUnitIndexInRow += 1;
      _this8.focusChaIndexInUnit = 0;
    };

    var combineLastText = function combineLastText() {
      var resTxt = "".concat(focusText).concat(newTxt);
      _this8.focusChaIndexInUnit += 1;
      rowData.splice(_this8.focusUnitIndexInRow, 1, {
        text: resTxt,
        type: _this8.getUnitType(resTxt)
      });
      return resTxt;
    };

    if (this.focusUnitIndexInRow === rowData.length - 1 && this.focusChaIndexInUnit === rowData[this.focusUnitIndexInRow].text.length - 1) {
      // console.log('是在行末尾添加');
      unitConnect(focusTxtType, txtType, function () {
        combineLastText();
      }, function () {
        newAppend();
      }); // 需要mention txt的条件
      ...
      if (getIsRegularType(txtType)) {
        if (getIsRegularType(focusTxtType)) {
          mentionText = "".concat(focusText).concat(newTxt);
        } else {
          mentionText = newTxt;
        }
      }
      ...
      var combineToOneUnit = function combineToOneUnit() {
        var resTxt = "".concat(focusTextHead).concat(newTxt).concat(focusTextTail);
        _this8.focusChaIndexInUnit += 1;
        rowData.splice(_this8.focusUnitIndexInRow, 1, {
          text: resTxt,
          type: _this8.getUnitType(resTxt)
      ...
      var combineNext = function combineNext() {
        // 把当前txt 和下一个unit txt合并
        var resTxt = "".concat(newTxt).concat(nextTxt);
        _this8.focusUnitIndexInRow += 1;
        _this8.focusChaIndexInUnit = 0;
        rowData.splice(_this8.focusUnitIndexInRow, 1, {
          text: resTxt,
      ...
      var handleSplitType = function handleSplitType() {
        if (getIsRegularType(txtType)) {
      ...
      if (focusTxtType === 'space') {
        ...
        if (txtType === 'space') {
          // 输入的是space
          combineToOneUnit();
        } else if (focusText.length - 1 > this.focusChaIndexInUnit) {
          // 当前输入的不是空 在空格间插入
          splitToThreeUnit();
          mentionText = newTxt;
        } else {
          // 当前输入的不是space 并且在空格末尾插入
...
EditorDataController_DataController.prototype.getCharacterWidth = function (str) {
  var _this9 = this;

  var width = 0;
  str.split('').forEach(function (value, index) {
    if (/[\u4e00-\u9fa5]/.test(value)) {
      width += _this9.CNWidth;
    } else {
      width += _this9.characterWidth;
    }
  });
  return width;
}; // 设置提示框的数据
...
EditorDataController_DataController.prototype.setMentionData = function (keyword) {
  if (keyword !== null) {
    // 此处需要添加 隔断符判断
    var list = keywordsCollection.find({
      text: {
        $contains: keyword
      }
    });
    var renderList = copy_default.a.deepCopy(list);
    var keywordLen = keyword.length;
    renderList.forEach(function (value, index) {
      // ...
      var splitText = [];
      var valueText = value.text;
      var keywordIndex = valueText.search(keyword);
...
EditorDataController_DataController.prototype.replaceFocusToMentionWord = function (item) {
  var focusRowIndex = this.focusRowIndex,
      focusUnitIndexInRow = this.focusUnitIndexInRow;
  var text = item.text;
  var unitData = this.textData[focusRowIndex][focusUnitIndexInRow];
...
EditorDataController_DataController.prototype.insertTxts = function (text) {
  if (text.length > 1) {
    this.addTxts(text);
...
EditorDataController_DataController.prototype.addTxts = function (newTxt) {
  var focusRowIndex = this.focusRowIndex,
      focusUnitIndexInRow = this.focusUnitIndexInRow,
      focusChaIndexInUnit = this.focusChaIndexInUnit;
...
EditorDataController_DataController.prototype.onDelete = function () {
  var _this10 = this;

  // console.log('deleteCharacter', this.focusUnitIndexInRow, this.focusChaIndexInUnit);
  var selectAreaLen = this.selectArea.length;
  ...
  if (selectAreaLen > 0) {
    ...
    var startFocusRowIndex = this.startMoveData.focusRowIndex;
    var startFocusUnitIndexInRow = this.startMoveData.focusUnitIndexInRow;
    var startFocusChaIndexInUnit = this.startMoveData.focusChaIndexInUnit;
    var _this$startMoveData = this.startMoveData,
        cursorLeft = _this$startMoveData.cursorLeft,
        cursorTop = _this$startMoveData.cursorTop;
    var focusRowData = textData[focusRowIndex]; // 当前聚焦的数据

    var focusUnitData = focusRowData[focusUnitIndexInRow];
    var startRowData = textData[startFocusRowIndex]; // 起始的数据

    var startUnitData = startRowData[startFocusUnitIndexInRow];

    var combineTxt = function combineTxt(headUnitData, headFocusChaIndexInUnit, tailUnitData, tailFocusChaIndexInUnit) {
      var txt = "".concat(headUnitData.text.slice(0, headFocusChaIndexInUnit + 1)).concat(tailUnitData.text.slice(tailFocusChaIndexInUnit + 1));
      headUnitData.text = txt;
      headUnitData.type = _this10.getUnitType(txt);
    };

    var setCursorToHead = function setCursorToHead() {
      _this10.cursorData.left = cursorLeft;
      _this10.cursorData.top = cursorTop;
      _this10.focusRowIndex = startFocusRowIndex;
      _this10.focusUnitIndexInRow = startFocusUnitIndexInRow;
      _this10.focusChaIndexInUnit = startFocusChaIndexInUnit;
    };

    if (selectAreaLen === 1) {
      // 只选中一行
      // 选中行末尾到中间
      var selectUnitDiff = focusUnitIndexInRow - startFocusUnitIndexInRow;
      var selectUnitDiffAbs = Math.abs(selectUnitDiff);
      console.log(selectUnitDiffAbs);

      if (selectUnitDiffAbs === 0) {
        // 只选中一个unit
        ...
        if (focusChaIndexInUnit > startFocusChaIndexInUnit) {
          // 光标恢复到起始位置
          setCursorToHead();
          combineTxt(focusUnitData, startFocusChaIndexInUnit, focusUnitData, focusChaIndexInUnit);
        } else {
          combineTxt(focusUnitData, focusChaIndexInUnit, focusUnitData, startFocusChaIndexInUnit);
        }
      } else {
        // 选中2个以上unit
        var deleteOneRowMoreUnit = function deleteOneRowMoreUnit(headUnitData, headFocusChaIndexInUnit, headFocusUnitIndex, tailUnitData, tailFocusChaIndexInUnit) {
          focusRowData.splice(headFocusUnitIndex + 1, selectUnitDiffAbs);
          combineTxt(headUnitData, headFocusChaIndexInUnit, tailUnitData, tailFocusChaIndexInUnit);
        };

        if (focusUnitIndexInRow < startFocusUnitIndexInRow) {
          deleteOneRowMoreUnit(focusUnitData, focusChaIndexInUnit, focusUnitIndexInRow, startUnitData, startFocusChaIndexInUnit);
        } else {
          deleteOneRowMoreUnit(startUnitData, startFocusChaIndexInUnit, startFocusUnitIndexInRow, focusUnitData, focusChaIndexInUnit);
          setCursorToHead();
        }
      }
    } else {
      // 选中两行以上
      var selectRowDiff = focusRowIndex - startFocusRowIndex;
      var selectRowDiffAbs = Math.abs(selectRowDiff);

      var deleteMuliline = function deleteMuliline(headUnitData, headFocusChaIndexInUnit, tailUnitData, tailFocusChaIndexInUnit, headRowData, headFocusUnitIndex, headFocusRowIndex, tailRowData, tailFocusUnitIndex) {
        var _headRowData;

        combineTxt(headUnitData, headFocusChaIndexInUnit, tailUnitData, tailFocusChaIndexInUnit);
        tailRowData.splice(0, tailFocusUnitIndex + 1);

        (_headRowData = headRowData).splice.apply(_headRowData, [headFocusUnitIndex + 1, headRowData.length].concat(toConsumableArray_default()(tailRowData)));
      ...
      if (focusRowIndex < startFocusRowIndex) {
        deleteMuliline(focusUnitData, focusChaIndexInUnit, startUnitData, startFocusChaIndexInUnit, focusRowData, focusUnitIndexInRow, focusRowIndex, startRowData, startFocusUnitIndexInRow);
      } else {
        deleteMuliline(startUnitData, startFocusChaIndexInUnit, focusUnitData, focusChaIndexInUnit, startRowData, startFocusUnitIndexInRow, startFocusRowIndex, focusRowData, focusUnitIndexInRow);
        setCursorToHead();
      }
    }
...
EditorDataController_DataController.prototype.onEnterClick = function () {
  var _this11 = this;

  console.log('onEnterClick', this.textData, this.focusUnitIndexInRow, this.focusChaIndexInUnit);

  var newRow = function newRow() {
    _this11.lineNumList.push(_this11.lineNumList.length + 1);

    _this11.focusUnitIndexInRow = -1;
    _this11.focusChaIndexInUnit = null;
    _this11.cursorData.top += _this11.lineHeight;
    _this11.cursorData.left = _this11.editLetPadding;
  };
...
EditorDataController_DataController.prototype.breakRow = function () {
...
EditorDataController_DataController.prototype.cursorMoveLeft = function () {
  var _this12 = this;

  this.resetArrowKeyStartIndex();

  var moveLeftCursor = function moveLeftCursor() {
    _this12.cursorData.left -= _this12.getCharacterWidth(_this12.textData[_this12.focusRowIndex][_this12.focusUnitIndexInRow].text[_this12.focusChaIndexInUnit]);
  };
...
EditorDataController_DataController.prototype.getArrowStartData = function () {
...
EditorDataController_DataController.prototype.cursorMoveCalculatePosition = function (focusUnitIndexInRow, compareLeft) {
...
EditorDataController_DataController.prototype.cursorMoveUp = function () {
  // 上一行有数据
  if (this.focusRowIndex > 0) {
...
EditorDataController_DataController.prototype.cursorMoveDown = function () {
  // 下一行有数据
...
EditorDataController_DataController.prototype.moveToNextRow = function () {
  if (this.textData[this.focusRowIndex + 1]) {
    this.cursorData.left = this.editLetPadding;
    this.cursorData.top += this.lineHeight;
    this.focusRowIndex += 1;
...
EditorDataController_DataController.prototype.cursorMoveRight = function () {
  var _this13 = this;
  ...
  var toNextUnit = function toNextUnit() {
    // 移动到下一个unit
    _this13.cursorData.left += _this13.getCharacterWidth(_this13.textData[_this13.focusRowIndex][_this13.focusUnitIndexInRow].text[_this13.focusChaIndexInUnit]);
    _this13.focusUnitIndexInRow += 1;
    _this13.focusChaIndexInUnit = 0;
  };

  var toNextCharactor = function toNextCharactor() {
    // 当前unit内移动一个字符
    _this13.cursorData.left += _this13.getCharacterWidth(_this13.textData[_this13.focusRowIndex][_this13.focusUnitIndexInRow].text[_this13.focusChaIndexInUnit]);
    _this13.focusChaIndexInUnit += 1;
  };
...
EditorDataController_DataController.prototype.copy = function (e) {
  var _this14 = this;

  var _this$startMoveData2 = this.startMoveData,
      focusRowIndex = _this$startMoveData2.focusRowIndex,
      focusUnitIndexInRow = _this$startMoveData2.focusUnitIndexInRow,
      focusChaIndexInUnit = _this$startMoveData2.focusChaIndexInUnit;
  ...
  var generateSingleRowData = function generateSingleRowData(frontRowIndex, // 头部focus参数
  frontFocusUnitIndexInRow, frontChaIndexInUnit, behindFocusUnitIndexInRow, // 尾部focus参数
  behindChaIndexInUnit) {
    var resUnitArr = [];
    var frontText = _this14.textData[frontRowIndex][frontFocusUnitIndexInRow].text;

    if (frontChaIndexInUnit < frontText.length - 1) {
      txt = frontText.slice(frontChaIndexInUnit + 1);
      resUnitArr.push({
        text: txt,
        type: _this14.getUnitType(txt)
      });
    }

    resUnitArr.push.apply(resUnitArr, toConsumableArray_default()(_this14.textData[frontRowIndex].slice(frontFocusUnitIndexInRow + 1, behindFocusUnitIndexInRow)));
    txt = _this14.textData[frontRowIndex][behindFocusUnitIndexInRow].text.slice(0, behindChaIndexInUnit + 1);
    resUnitArr.push({
      text: txt,
      type: _this14.getUnitType(txt)
    });
  ...
  var generateMultiRowData = function generateMultiRowData(frontRowIndex, frontFocusUnitIndexInRow, frontFocusChaIndexInUnit, behindFocusRowIndex, behindFocusUnitIndexInRow, behindFocusChaIndexInUnit) {
    var _resUnitArr;

    var resUnitArr = [];

    if (frontFocusUnitIndexInRow === -1) {
      if (_this14.textData[frontRowIndex].length === 0) {
        resUnitArr = [];
      }
    } else if (_this14.textData[frontRowIndex][frontFocusUnitIndexInRow].text.length - 1 > frontFocusChaIndexInUnit) {
      txt = _this14.textData[frontRowIndex][frontFocusUnitIndexInRow].text.splice(frontFocusChaIndexInUnit + 1);
      resUnitArr.push({
        text: txt,
        type: _this14.getUnitType(txt)
      });
    ...
    (_resUnitArr = resUnitArr).push.apply(_resUnitArr, toConsumableArray_default()(_this14.textData[frontRowIndex].slice(frontFocusUnitIndexInRow + 1)));

    resRowArr.push(resUnitArr);
    resRowArr.push.apply(resRowArr, toConsumableArray_default()(_this14.textData.slice(frontRowIndex + 1, behindFocusRowIndex)));
    resUnitArr = [];

    if (behindFocusUnitIndexInRow !== -1) {
      var _resUnitArr2;

      (_resUnitArr2 = resUnitArr).push.apply(_resUnitArr2, toConsumableArray_default()(_this14.textData[behindFocusRowIndex].slice(0, behindFocusUnitIndexInRow)));

      txt = _this14.textData[behindFocusRowIndex][behindFocusUnitIndexInRow].text.slice(0, behindFocusChaIndexInUnit + 1);
      resUnitArr.push({
        text: txt,
        type: _this14.getUnitType(txt)
      });
  ...
  if (tFocusRowIndex > focusRowIndex) {
    // 向下行选中
    generateMultiRowData(focusRowIndex, focusUnitIndexInRow, focusChaIndexInUnit, tFocusRowIndex, tFocusUnitIndexInRow, tFocusChaIndexInUnit);
  } else if (tFocusRowIndex < focusRowIndex) {
    // 向上行选中
    generateMultiRowData(tFocusRowIndex, tFocusUnitIndexInRow, tFocusChaIndexInUnit, focusRowIndex, focusUnitIndexInRow, focusChaIndexInUnit);
  } else {
    // 在同一行
    if (tFocusUnitIndexInRow > focusUnitIndexInRow) {
      // 往右侧选中不同unit
      generateSingleRowData(focusRowIndex, focusUnitIndexInRow, focusChaIndexInUnit, tFocusUnitIndexInRow, tFocusChaIndexInUnit);
    } else if (tFocusUnitIndexInRow < focusUnitIndexInRow) {
      // 往左侧选中不同unit
      console.log('往左侧选中不同unit起始参数', focusUnitIndexInRow, focusChaIndexInUnit);
      generateSingleRowData(focusRowIndex, tFocusUnitIndexInRow, tFocusChaIndexInUnit, focusUnitIndexInRow, focusChaIndexInUnit);
    } else {
      // 同一个unit
      var startText = this.textData[focusRowIndex][focusUnitIndexInRow].text;
...
EditorDataController_DataController.prototype.paste = function (pasteTxt) {
...
EditorDataController_DataController.prototype.insertUnitArr = function (copyRowData, focusRowIndex, focusUnitIndexInRow, focusChaIndexInUnit) {
  var _this15 = this;
    ...
    if (focusUnit) {
      unitConnect(copyLastUnit.type, focusUnit.type, function () {
        ...
        if (unitTextTail) {
          ...
          resUnitArr.push({
            text: unitTextTail,
            type: _this15.getUnitType(unitTextTail)
          });
        }
      });
    } else {
  ...
  if (copyRowDataLen >= 2) {
    var _resUnitArr3, _this$textData$focusR;

    // 粘贴板数据2行以上
    handleFirstUnit(); // 处理第一行剩下的unit

    (_resUnitArr3 = resUnitArr).push.apply(_resUnitArr3, toConsumableArray_default()(copyFirstRow.slice(1))); // 保留 当前focus行 尾部unit列


    var focusRowTail = this.textData[focusRowIndex].slice(focusUnitIndexInRow + 1, focusRowLen); // 插入新的第一行

    (_this$textData$focusR = this.textData[focusRowIndex]).splice.apply(_this$textData$focusR, [focusUnitIndexInRow, focusRowLen].concat(toConsumableArray_default()(resUnitArr)));

    var hasDataLastIndexRow = copyRowDataLen - 1; // while (copyRowData[hasDataLastIndexRow].length === 0) {
    //   hasDataLastIndexRow -= 1;
    // }

    if (copyRowData[hasDataLastIndexRow].length > 0) {
      var _resUnitArr4;

      // 最后一行有数据
      // 处理末行
      copyLastRow = copyRowData[hasDataLastIndexRow]; // 处理粘贴板最后一行前面unit
      ...
      (_resUnitArr4 = resUnitArr).push.apply(_resUnitArr4, toConsumableArray_default()(focusRowTail));
    } else {
      // 最后一行没数据
    ...
    if (copyFirstRowLen >= 2) {
      ...
      (_resUnitArr5 = resUnitArr).push.apply(_resUnitArr5, toConsumableArray_default()(copyFirstRow.slice(1, -1))); // 处理最后一个unit
  ...
  setTimeout(function () {
    _this15.focusUnitByIndex(newFocusRowIndex, newFocusUnitIndexInRow, newFocusChaIndexInUnit);
  }, 0);
}; // 获取内容


EditorDataController_DataController.prototype.getContent = function () {
  var content = '';
  var rowLen = this.textData.length;
  this.textData.forEach(function (value, index) {
    value.forEach(function (valueU, indexU) {
      content = "".concat(content).concat(valueU.text);
    });
...
/* harmony default export */ var EditorDataController = ({
  DataController: EditorDataController_DataController
});
...
/* harmony default export */ var Editvue_type_script_lang_js_ = ({
  ...
  data: function data() {
    return {
      ...
      dataController: new EditorDataController.DataController({})
    };
  },
  computed: {},
  watch: {
  ...
  methods: {
    ...
    getContent: function getContent() {
      return this.dataController.getContent();
    },
